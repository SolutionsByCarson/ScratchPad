...
- **Combat:** Fruit Area2D ([scripts/fruit.gd](scripts/fruit.gd)) is a small black circle (radius 2.5, half the grenade's size); flies horizontally at 250 px/s, despawns on walls, destroys "enemy" Area2Ds with `explosion` SFX. Grenade Area2D ([scripts/grenade.gd](scripts/grenade.gd)) is a red-orange circle (radius 5); launches in a ballistic arc (200 px/s horizontal, -260 px/s vertical, GRAVITY 700) from K input; explodes on first body contact or after 4s lifetime; AOE within 48 px destroys all "enemy" Area2Ds + plays `explosion` SFX. Both projectiles spawn fading afterimage trails. Slime ([scripts/slime.gd](scripts/slime.gd)) state-machine AI (WANDER → CHASE → WIND_UP → JUMP → LAND): aggros only when player is within `aggro_x_range` AND `aggro_y_tolerance`, persists 30s after last sighting, jump-attacks (with squash/stretch and 4px wall-collision lookahead so it slides down on impact), can leap off platforms and fall via gravity to ground below, ledge-avoids during wander/chase, blocked by walls, raycasts exclude the player, auto-falls if patrolling without ground. Configurable per-instance via @export. Purple variant: [scenes/slime_purple.tscn](scenes/slime_purple.tscn).
